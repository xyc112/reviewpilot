...
(4, '居远谋', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'USER'),
(5, '钟定博', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'USER'),
(6, '杨舒文', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'USER'),
(7, '谢宇成', '$2a$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'USER');
...
(16, 6, 5, '虚拟内存的作用', '虚拟内存是操作系统中的重要概念，但我不太理解它的具体作用。\n\n虚拟内存是如何实现的？它解决了什么问题？', '2024-03-10 09:30:00', '2024-03-10 09:30:00'),
(17, 7, 6, 'HTTP/2相比HTTP/1.1的改进', 'HTTP/2相比HTTP/1.1有哪些主要改进？\n\n多路复用、头部压缩、服务器推送等特性是如何提升性能的？', '2024-03-11 13:00:00', '2024-03-11 13:00:00'),
(18, 8, 7, 'CPU流水线技术', 'CPU流水线技术是如何提高处理效率的？\n\n流水线中的冒险（hazard）有哪些类型？如何解决这些冒险？', '2024-03-12 10:00:00', '2024-03-12 10:00:00');

-- 插入更多社区评论（使用新添加的用户）
INSERT INTO comment (id, post_id, author_id, content, parent_id, created_at, updated_at) VALUES
(15, 11, 5, 'ArrayList适合随机访问，LinkedList适合频繁插入删除。HashMap适合键值对存储，需要快速查找的场景。', NULL, '2024-03-05 10:00:00', '2024-03-05 10:00:00'),
(16, 11, 6, '补充：HashSet适合去重，TreeSet适合需要有序的场景。选择集合时主要考虑：是否需要有序、是否需要去重、访问模式等。', NULL, '2024-03-05 10:30:00', '2024-03-05 10:30:00'),
(17, 11, 7, '还要注意线程安全，如果多线程环境，可以使用ConcurrentHashMap、CopyOnWriteArrayList等。', 15, '2024-03-05 11:00:00', '2024-03-05 11:00:00'),
(18, 12, 4, '依赖注入的核心思想是控制反转（IoC）。@Autowired会从Spring容器中查找匹配的Bean并注入。', NULL, '2024-03-06 11:00:00', '2024-03-06 11:00:00'),
(19, 12, 6, '推荐使用构造函数注入，因为：1. 强制依赖，避免空指针；2. 便于测试；3. 不可变对象。字段注入不推荐，因为难以测试。', NULL, '2024-03-06 11:30:00', '2024-03-06 11:30:00'),
(20, 12, 7, 'Spring官方也推荐构造函数注入，这是最佳实践。', 19, '2024-03-06 12:00:00', '2024-03-06 12:00:00'),
(21, 13, 4, '二分查找的关键是保持循环不变式：搜索区间[left, right]始终包含目标元素（如果存在）。', NULL, '2024-03-07 15:00:00', '2024-03-07 15:00:00'),
(22, 13, 5, 'mid的计算：mid = left + (right - left) / 2 可以避免溢出。边界处理：left = mid + 1 和 right = mid - 1 要一致。', NULL, '2024-03-07 15:30:00', '2024-03-07 15:30:00'),
(23, 13, 7, '推荐使用左闭右开区间 [left, right)，这样边界处理更统一，不容易出错。', 22, '2024-03-07 16:00:00', '2024-03-07 16:00:00'),
(24, 14, 4, 'React组件通信方式：\n1. Props：父子组件\n2. Context：跨层级组件\n3. Redux/Zustand：全局状态管理\n4. 事件总线：兄弟组件', NULL, '2024-03-08 12:00:00', '2024-03-08 12:00:00'),
(25, 14, 5, '简单场景用Props，复杂场景用Context或状态管理库。Redux适合大型应用，Zustand更轻量。', NULL, '2024-03-08 12:30:00', '2024-03-08 12:30:00'),
...
(27, 15, 7, 'MySQL默认是REPEATABLE READ，PostgreSQL默认是READ COMMITTED。选择时要平衡一致性和性能。', NULL, '2024-03-09 16:30:00', '2024-03-09 16:30:00'),
(28, 16, 4, '虚拟内存将物理内存和磁盘存储结合起来，为每个进程提供独立的地址空间。通过页表实现地址转换。', NULL, '2024-03-10 10:30:00', '2024-03-10 10:30:00'),
(29, 16, 6, '虚拟内存解决了：1. 内存保护；2. 内存共享；3. 更大的地址空间；4. 简化内存管理。', NULL, '2024-03-10 11:00:00', '2024-03-10 11:00:00'),
(30, 17, 4, 'HTTP/2的主要改进：\n1. 多路复用：一个连接处理多个请求\n2. 头部压缩：HPACK算法\n3. 服务器推送：主动推送资源\n4. 二进制分帧：提高解析效率', NULL, '2024-03-11 14:00:00', '2024-03-11 14:00:00'),
(31, 17, 5, 'HTTP/2解决了HTTP/1.1的队头阻塞问题，大幅提升了性能，特别是在高延迟网络环境下。', NULL, '2024-03-11 14:30:00', '2024-03-11 14:30:00'),
...
(34, 18, 6, '现代CPU还使用乱序执行、超标量等技术进一步提升性能。', 33, '2024-03-12 12:00:00', '2024-03-12 12:00:00');
