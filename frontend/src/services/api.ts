...
export const courseAPI = {
    ...
    getCourse: (courseId: number) => api.get<ApiResponse<any>>(`/courses/${courseId}`),
    createCourse: (courseData: any) => api.post<ApiResponse>('/courses', courseData),
    enroll: (courseId: number) => api.post<ApiResponse>(`/courses/${courseId}/enroll`),
}
...
export const userAPI = {
    getScores: (params?: any) => api.get<ApiResponse<any>>('/user/scores', { params }),
    batchImportScores: (scores: any[]) =>
        api.post<ApiResponse>('/user/scores/batch', scores),
    getLearningProgress: (courseId: number) =>
        api.get<ApiResponse<any>>('/user/learning-progress', { params: { courseId } }),
...
export const notesAPI = {
    ...
    updateNote: (pointId: number, noteData: any) =>
        api.put<ApiResponse>(`/user/knowledge-points/${pointId}/user-note`, noteData),
    deleteAttachment: (noteId: number, attachmentId: number) =>
        api.delete<ApiResponse>(`/user-notes/${noteId}/attachments/${attachmentId}`),
}
...
export const quizAPI = {
    ...
    getWrongQuestions: (courseId: number) =>
        api.get<ApiResponse<any>>('/quiz/wrong-questions', { params: { courseId } }),
}

export const analyticsAPI = {
    getLearningStats: (courseId: number) =>
        api.get<ApiResponse<any>>('/analytics/learning-stats', { params: { courseId } }),
}