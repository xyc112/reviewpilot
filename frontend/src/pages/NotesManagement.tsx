...
import { Card, Row, Col, List, Button, Input, Typography, Tag, Modal, Upload, message } from 'antd'
import {
    EditOutlined,
    DeleteOutlined,
    PlusOutlined,
...
const NotesManagement: React.FC = () => {
            ...
            const mockNotes: Note[] = [
                {
                    noteId: 1,
                    pointId: 1,
                    content: '面向对象编程的四大特性：封装、继承、多态、抽象。封装可以隐藏实现细节，提高代码安全性。',
                    attachments: [
                        {
                            fileId: 1,
    ...
    const handleSelectPoint = (point: KnowledgePoint) => {
        ...
        const existingNote = notes.find(note => note.pointId === parseInt(point.id))
        if (existingNote) {
        ...
        <div css={notesStyles.container}>
            ...
            <Row gutter={[16, 16]}>
                        ...
                        <List
                            ...
                            renderItem={(point) => (
                                <List.Item
                                    ...
                                    style={{
                                        background: selectedPoint?.id === point.id ? '#f0f8ff' : 'transparent',
                                        cursor: 'pointer'
                                    }}
                                    ...
                                    <List.Item.Meta
                                        ...
                                        title={
                                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                                ...
                                                <Tag color={
                                                    point.masteryLevel === 'strong' ? 'green' :
                                                        point.masteryLevel === 'medium' ? 'orange' : 'red'
                                                }>
                                                    {point.masteryLevel === 'strong' ? '掌握' :
                                                        point.masteryLevel === 'medium' ? '一般' : '薄弱'}
                                                </Tag>
                                            </div>
                                        }
                                        description={
                ...
                <Col xs={24} lg={16}>
                    <Card
                        title={
                            selectedPoint ? `笔记编辑 - ${selectedPoint.name}` : '选择知识点开始编辑'
                        }
                        css={notesStyles.card}
                        extra={
                            selectedPoint && (
                    ...
                    {/* 历史笔记 */}
                    {notes.filter(note => note.pointId === parseInt(selectedPoint?.id || '0')).length > 0 && (
                        <Card title="历史笔记" css={notesStyles.card} style={{ marginTop: 16 }}>
                            <List
                                dataSource={notes.filter(note => note.pointId === parseInt(selectedPoint?.id || '0'))}
                                renderItem={(note) => (
                                    <List.Item
                                        actions={[
                                            <Button
